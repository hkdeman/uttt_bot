...
import numpy as np
from tactics.algorithm import MCTS
...
class NMCTS(StdOutPlayer):
    ...
    def get_my_move(self):  # -> Tuple[MainBoardCoords, SubBoardCoords]
        move = None
        if self.total_moves < 7:
            move = self.get_my_move_nn()
        else:
            move = self.get_my_move_mcts()            

        self.total_moves += 1
        return move

    def get_my_move_mcts(self):  # -> Tuple[MainBoardCoords, SubBoardCoords]
    ...
    def get_my_move_nn(self):  # -> Tuple[MainBoardCoords, SubBoardCoords]
        current_board = parse(self.main_board)
        last_turn = self.main_board.sub_board_next_player_must_play
        features = [] # so you can map 
        all_moves = [] # these two by indexes
